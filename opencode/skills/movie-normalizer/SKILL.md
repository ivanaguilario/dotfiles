---
name: movie-normalizer
description: Plans and safely remuxes movie files to keep only the intended English tracks with canonical title-year filenames
...
- Inspect movie video files in a user-provided input directory.
- Build a conservative remux plan for canonical `<title> (YYYY).ext` output files.
- When explicitly instructed, write cleaned copies to a user-provided output directory.

## When to use me

Use this skill when the user wants to audit or safely remux movie files so each output keeps only the intended English streams and uses canonical movie filenames.
...
- Never guess when stream selection or filename normalization is ambiguous.
...
- `input_dir`: directory containing source movie files to inspect
- `output_dir`: directory where cleaned outputs will be written

## Optional user inputs

- Movie filter or explicit title/year mapping
...
- Every output filename must match `^.+ \(\d{4}\)\.[A-Za-z0-9]+$`.
- `<title>` is required and must be non-empty.
- `YYYY` is required and must be a 4-digit year.
- Preserve meaningful edition, version, or cut labels as part of `<title>`.
- Examples of meaningful labels to preserve include `Director's Cut`, `Extended Edition`, `Final Cut`, `Theatrical Cut`, `Unrated`, `Remastered`, and similar movie-version text.
- Remove bracketed or dotted technical release tags only when they are clearly junk and not part of the movie title.
- Examples of technical junk include source, codec, resolution, dynamic range, audio format, release group, and container tags such as `BluRay`, `WEB-DL`, `WEBRip`, `1080p`, `2160p`, `x264`, `x265`, `HEVC`, `HDR`, `DV`, `DTS`, `TrueHD`, `Atmos`, and similar technical metadata.
- Normalize common separators in the inferred title to readable spaces, but do not rewrite or invent title words.
- If a source filename cannot be confidently mapped to `<title> (YYYY).ext`, stop and ask.
- If more than one plausible title/year interpretation exists, stop and ask.
- Never invent a title, edition, cut label, or year.
...
- Forced English subtitle is default when both regular and forced subtitles are present.
- A lone regular English subtitle is not default.
- Output filename matches `<title> (YYYY).ext`.

## Failure and stop conditions

Stop and ask when any of the following occur:

- Source filename cannot be confidently normalized to `<title> (YYYY).ext`.
- Source filename lacks a confidently identified 4-digit movie year.
- More than one plausible title/year interpretation exists.
- Meaningful title text might be removed as technical junk.
